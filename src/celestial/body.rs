...
pub trait Body {
	...
	fn size(&self) -> f64;
	/// The mass of the body in metric tonnes.
	fn mass(&self) -> f64;
	...
	// Usually it should be fine to leave the default and not implement this function seperately.
	fn density(&self) -> f32 {
		// `self.mass()` yields the weight tonnes. To get kg we divide by 1000.0
		// `self.volume()` yields the volume in km^3. To get m^3 we multiply it by 1000.0^3
		// Hence we divide by 1000^2 in total.
		(self.mass() / self.volume() as f64 / 1000000.0) as f32
	}
